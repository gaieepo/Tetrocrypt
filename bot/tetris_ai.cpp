        ...
        */
        ...
        // �㷽��
        {
            ...
            // ��������
            {
                ...
                if ( pool.b2b ) s -= (int) ((double)(ai_param.b2b * 5) / (1 + (TSD_only? 0 : std::pow(5, avg_height - 6.5)))) + 2; // b2b score
                if ( t_clear > 0 ) {
...
#define BEG_ADD_Y 1
    MovingSimple AISearch(AI_Param ai_param, const GameField& pool, int hold, Gem cur, int x, int y, const std::vector<Gem>& next, bool canhold, int upcomeAtt, int maxDeep, int & searchDeep) {
...

        {
            ...
            for (MovingList::iterator it = movs.begin(); it != movs.end() && Thinking; ++it) {
                ++search_nodes;
        ...
        for (; /*search_nodes < max_search_nodes &&*/ depth < maxDeep; searchDeep = ++depth ) { //d < maxDeep
                ...
                --pq_size, pq_last->dec_size())
            {
                ...
                if ( movs.empty() ) {
                    ...
                    for (size_t i = 0; (i < movs.size()) && Thinking; ++i) {
                        ++search_nodes;
                    ...
                    for ( int i = 0; i < seach_select_best && ! p.empty() && Thinking; ++i) {
                        pq->push(p.front());
                        p.pop_back();
                        p.dec_size();
                    }
                }
                if ( canhold && depth + next_add < next.size() && Thinking)
                {
                    MovsState ms_last = pq_last->back();
        ...
        //if (0)
        if ( ai_settings[player].hash && canhold && Thinking ) { // extra search
            std::swap(pq_last, pq);
            pq->clear();
	...
	int score_avoid_softdrop(int param, bool sd, int cur, bool wk, double h) {
		return TSD_only? 0 : (int) ((double)((sd && !((cur == AI::GEMTYPE_T || isEnableAllSpin()) && wk))? param * 5 : 0) / (1 + std::pow(5, h - 6.5)));
	}
