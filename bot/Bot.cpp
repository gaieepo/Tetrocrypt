...
void Bot::updateStyle(int* param) {
  globalparam.miny_factor = param[0];
  globalparam.hole = param[1];
  globalparam.open_hole = param[2];
  globalparam.v_transitions = param[3];
  globalparam.tspin3 = param[4];

  globalparam.clear_efficient = param[5];
  globalparam.upcomeAtt = param[6];
  globalparam.h_factor = param[7];
  globalparam.hole_dis_factor2 = param[8];
  globalparam.hole_dis = param[9];

  globalparam.hole_dis_factor = param[10];
  globalparam.tspin = param[11];
  globalparam.hold_T = param[12];
  globalparam.hold_I = param[13];
  globalparam.clear_useless_factor = param[14];

  globalparam.dif_factor = param[15];
  globalparam.b2b = param[16];
  globalparam.combo = param[17];
  globalparam.avoid_softdrop = param[18];
  globalparam.tmini = param[19];

  globalparam.strategy_4w = param[20];
}
...
std::string Bot::outputAction(char* str, int len) {
    ...
    cerr<< " " <<((canhold)?"canhold":"NOThold")<<", Deep:"<<deep<<",last:"<<tetris.ai_last_deep<<endl;
    #endif
